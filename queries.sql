...
-- Every album
SELECT * FROM albums; -- remember the colon at the end, it won't work otherwise!
                      -- press q to exit scrolling interface
                      -- this has id, title, and artist id

-- Every album title (and only the title)
SELECT title FROM albums; -- this has only the title, no id or artist id
...
SELECT * FROM invoices ORDER BY total LIMIT 10; -- find __ least or most by ordering, then limiting amount outputted
...
SELECT billing_country, COUNT(*) FROM invoices GROUP BY billing_country ORDER BY COUNT(*) DESC LIMIT 5;;
  -- must use GROUP BY if you select something specific
